...
def get_historical_prices(days=1):
    ...
    try:
        ...
        # Create simplified history data points
        now = datetime.now()
        timestamps = [now - timedelta(hours=x) for x in range(24, -1, -1)]
        
        # Calculate approximate prices based on current price and change
        price_change_ratio = price_change / 100
        price_24h_ago = current_price / (1 + price_change_ratio)
        
        # Create a simple linear interpolation between 24h ago price and current price
        prices = []
        for i in range(25):
            ratio = i / 24
...
def get_current_prices():
    ...
    try:
        ...
        # Calculate UYU price using a fixed conversion rate
        uyu_rate = 39.5  # Fixed conversion rate
